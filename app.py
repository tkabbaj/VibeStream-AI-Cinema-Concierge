...
# 1. Attempt to retrieve the API Key from automated sources.
# Priority: 1. Environment Variables (Docker/Cloud) -> 2. Streamlit Secrets (Cloud/Local)
api_key = os.getenv("GOOGLE_API_KEY")

if not api_key:
    try:
        # Check Streamlit's internal secrets manager if environment variable is missing
        if "GOOGLE_API_KEY" in st.secrets:
            api_key = st.secrets["GOOGLE_API_KEY"]
    except Exception:
        # Fallback gracefully if secrets are not available in this environment
        pass

# 2. If the key is still missing, prompt the user for manual input in the sidebar
if not api_key:
    # Sidebar configuration for authentication
    st.sidebar.title("Configuration")
    api_key = st.sidebar.text_input("Enter Google AI Studio API Key:", type="password")

# 4. Guard: Stop execution if no key is found
if not api_key:
    st.info("Get a free API Key from Google AI Studio and paste it in the sidebar to initialize the agent.")
    st.stop()

# --- APP LOGIC (Only runs if api_key is present) ---
